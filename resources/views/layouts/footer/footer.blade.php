<!-- begin:: Footer -->
 <div class="kt-footer  kt-grid__item kt-grid kt-grid--desktop kt-grid--ver-desktop kt-footer--fixed" id="kt_footer">
     <div class="kt-container  kt-container--fluid ">
            <div class="kt-footer__copyright">

                2019&nbsp;©&nbsp;
                
                <a href="javascript:void(0)" class="kt-link">DGTC</a>
            </div>
            
            <div class="kt-aside__brand-logo kt-hidden-mobile center_img_foot" >
                <button type="button" class="btn btn-outline-danger" 
                style="width: 90px;              position: static;
                margin-top: 20px;
                margin-left: 10px;
                margin-right: 20px;
            ">
                <a href="javascript:window.print()" >Imprimir</a> </button>
               <a href="{{ url('/admin') }}">
               <img alt="Logo" src="{{ URL::asset('assets/media/company-logos/logotipo_SAF-01.svg')}}" class="kt-footer__brand-logo-default" width="350"
               style="padding-top: 13px;width: 320px;">
               </a>
            </div>
            

         <div class="kt-footer__menu">
             
         </div>
     </div>
 </div>
 <!-- end:: Footer -->